...
(prn (pods/load-pod "./pod-babashka-go-duckdb"))

(require '[pod.babashka.go-duckdb :as duckdb])

(def temp-file (str (fs/file (fs/temp-dir) "foo.db")))

(fs/delete-if-exists temp-file)

(prn (duckdb/execute! temp-file ["create table if not exists foo (the_text TEXT, the_int INTEGER, the_real REAL, the_blob BLOB, the_json JSON)"]))
(prn (duckdb/execute! temp-file ["delete from foo"]))
...
(testing "multiple results"
  (prn (duckdb/execute! temp-file
                        ["insert into foo (the_text, the_int, the_real) values (?,?,?), (?,?,?)"
                         "bar" 3 1.5
                         "baz" 4 1.5])))

(def results (duckdb/query temp-file ["select * from foo order by the_int asc"]))
...
(def direct-results (duckdb/query temp-file "select * from foo order by the_int asc"))
...
(def json-field-result (duckdb/query temp-file ["select the_json->>'$.bar' as bar from foo where the_json is not null"]))
